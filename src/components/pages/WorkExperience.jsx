import React from "react";

const WorkExperience = ({ pageChange }) => {
  return (
    <div>
      <h1 className=" font-heading font-bold text-center mb-2 ">
        Work Experience
      </h1>
      <div className=" border-s-2 border-sky-500 px-[10px] relative ">
        <div className=" w-3 h-3 bg-sky-500 rounded-full top-0 absolute -left-[7px]"></div>
        <div>
          <h2 className=" flex text-xs text-sky-500">
            <span>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                strokeWidth={1.5}
                stroke="currentColor"
                className="w-4 h-4"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 0 1 2.25-2.25h13.5A2.25 2.25 0 0 1 21 7.5v11.25m-18 0A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75m-18 0v-7.5A2.25 2.25 0 0 1 5.25 9h13.5A2.25 2.25 0 0 1 21 11.25v7.5m-9-6h.008v.008H12v-.008ZM12 15h.008v.008H12V15Zm0 2.25h.008v.008H12v-.008ZM9.75 15h.008v.008H9.75V15Zm0 2.25h.008v.008H9.75v-.008ZM7.5 15h.008v.008H7.5V15Zm0 2.25h.008v.008H7.5v-.008Zm6.75-4.5h.008v.008h-.008v-.008Zm0 2.25h.008v.008h-.008V15Zm0 2.25h.008v.008h-.008v-.008Zm2.25-4.5h.008v.008H16.5v-.008Zm0 2.25h.008v.008H16.5V15Z"
                />
              </svg>
            </span>
            <span>2023 - 2024</span>
          </h2>
          <h1 className=" font-heading font-bold text-sm ">
            Frontend Developer(MMS IT WorkShop)
          </h1>
          <div className=" text-xs leading-[0.8rem]">
            <ol className=" text-sm grid grid-cols-2">
              <li>1. Touche</li>
              <li>2. Disyuting</li>
              <li>3. Dashboard</li>
              <li>4. E-commerce</li>
            </ol>
          </div>
        </div>
      </div>

      <div className=" border-s-2 border-sky-500 px-[10px] relative ">
        <div className=" w-3 h-3 bg-sky-500 rounded-full top-0 absolute -left-[7px]"></div>
        <div>
          <h2 className=" flex text-xs text-sky-500 ">
            <span>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                strokeWidth={1.5}
                stroke="currentColor"
                className="w-4 h-4"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 0 1 2.25-2.25h13.5A2.25 2.25 0 0 1 21 7.5v11.25m-18 0A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75m-18 0v-7.5A2.25 2.25 0 0 1 5.25 9h13.5A2.25 2.25 0 0 1 21 11.25v7.5m-9-6h.008v.008H12v-.008ZM12 15h.008v.008H12V15Zm0 2.25h.008v.008H12v-.008ZM9.75 15h.008v.008H9.75V15Zm0 2.25h.008v.008H9.75v-.008ZM7.5 15h.008v.008H7.5V15Zm0 2.25h.008v.008H7.5v-.008Zm6.75-4.5h.008v.008h-.008v-.008Zm0 2.25h.008v.008h-.008V15Zm0 2.25h.008v.008h-.008v-.008Zm2.25-4.5h.008v.008H16.5v-.008Zm0 2.25h.008v.008H16.5V15Z"
                />
              </svg>
            </span>
            <span>2024 - 2025</span>
          </h2>
          <h1 className=" font-heading font-bold text-sm ">On site Projects</h1>
          <div className=" text-xs leading-[0.8rem]">
            <ul className=" grid grid-cols-1 text-sm ">
              <li>1.Baranie(user)</li>
              <li>2.Baranie Management(dashboard)</li>
              <li>3.Baranie mobile(user)</li>
              <li>3.Baranie Management mobile</li>
            </ul>
          </div>
        </div>
      </div>

      <div className=" border-s-2 border-sky-500 px-[10px] relative ">
        <div className=" w-3 h-3 bg-sky-500 rounded-full top-0 absolute -left-[7px]"></div>
      </div>

      {/* page No  */}
      <div className=" flex justify-around text-xs absolute bottom-3 w-full text-center">
        <div></div>
        <div>1</div>
        <button
          onClick={pageChange}
          className="bg-gray-300 rounded-lg size-4
         "
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            strokeWidth={1.5}
            stroke="currentColor"
            className="w-4 h-4"
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              d="m8.25 4.5 7.5 7.5-7.5 7.5"
            />
          </svg>
        </button>
      </div>
    </div>
  );
};

export default WorkExperience;
